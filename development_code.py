...
#                 recv, err = sx.recv(len=0, timeout_en=True, timeout_ms=200)
#                 if recv:
#                     try:
...
# #test 2 with chat message application
# from sx1262 import SX1262
# import network
...
#                 if recv:
#                     try:
#                         if recv.decode().strip() == f"ACK{i+1}":
#                             print(f"[ACK] Received ACK for chunk {i+1}")
#                             cheak = True
#                             break  # Break out of ACK waiting loop
#                     except:
#                         pass  # Ignore decode errors
...
#             headers = f"HTTP/1.1 200 OK\r\nContent-Disposition: attachment; filename={file_name}\r\nContent-Type: application/octet-stream\r\nContent-Length: {len(received_file)}\r\n\r\n"
#             conn.send(headers.encode())
#             conn.send(received_file)
...
# #test 3 with correct pin declaration and different range settings
from sx1262 import SX1262
import network
import usocket as socket
